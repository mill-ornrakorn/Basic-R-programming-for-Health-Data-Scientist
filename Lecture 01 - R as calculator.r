...
#                 Basic R programming
#===============================================================
# TITLE   : Lecture 01 - R as calculator
...
y = 0:10 # จาก 0 ถึง 10 เพิ่มทีละ 1 
y

z = c(3,1,4,1,5,9,3) #เป็น Vector โดย c ย่อมาจาก เชื่อมเข้าด้วยกัน(concatenate)
z
...
for (i in 1:6) {
  if(x[i]>=0)y[i]=sqrt(x[i]) else y[i]=sqrt(-x[i])
}
...
vec4 <- gl(4, 1, 4, label = c("l1", "l2", "l3", "l4"))  # gl = generate levels โดย factor with 4 levels
vec4
...
# initialised vector without assign value จึงใส่ 0 มาหมดเลย
...
## Sorting vector
sort(vec1)  # ascending sort  น้อยไปมาก
sort(vec1, decreasing = TRUE)  # Descending sort  มากไปน้อย
...
matrix(v)
matrix(v, nrow = 2)
matrix(1:12, byrow = TRUE, nrow = 4, ncol = 3)
